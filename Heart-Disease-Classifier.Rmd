...
# Methods
The heart disease data set contains 303 observations with some missing values, so complete case analysis was used to reduce the number of observations to 297. Table \ref{tab:data} contains a written description of the fourteen variables and Table \ref{tab:data2} contains a detailed summary of the data set. The response variable is `num` - the diagnosis of heart disease (yes/no), while the remaining thirteen variables are the predictors. The response variable along with the thirteen predictors are used in each of the classification methods. 
...
cleveland <- read.csv("cleveland.txt", header = F, na.strings = "?")
...
cbind(variables,description) %>%
  kable(caption = "\\label{tab:data}Heart Disease Data Set",
...
cbind(whole,by_num) %>%
  print(caption = "\\label{tab:data2}Detailed Summary of the Heart Disease Data Set",
...
Based on the assessment of the performance of the three classification methods, a LASSO-penalized logistic regression model was then trained to be used for classification of future observations. Previously, the data was split into training and test sets, however this model was trained on the entire data set because it will be used to predict on future observations which can be treated as our test set, as the model has not seen this new data. The coefficients of the predictors are shown in Table \ref{tab:lasso} below. It is interesting to note that the coefficients of the predictors `age`, `cp3`, `slope3` and `thal6` have been shrunk to zero. There are also several coefficients that are close to zero, most notably  `trestbps`, `chol`, `restecg1`, `thalach`. The descriptions for these variables can be found in Table \ref{tab:data}. 
...
coef(lr.final, s = "lambda.min") %>%
  ...
  kable(caption = "\\label{tab:lasso}LASSO-penalized logistic regression model to predict future observations")
```
...
# Table of minimum lambdas
data.frame(Lambda = round(lasso.lambda, 3)) %>%
  rownames_to_column() %>%
  pivot_longer(-rowname, names_to = "Run") %>%
  pivot_wider(names_from = rowname) %>%
  kable(caption = "\\label{tab:lambda}Minimum values of lambda across 5 runs",
        align = "c") 
```
...
data.frame(k = knn.k) %>%
  ...
  pivot_longer(-rowname, names_to = "Run") %>%
  pivot_wider(names_from = rowname) %>%
  kable(caption = "\\label{tab:k}Optimal values of k across 5 runs",
        align = "c") 
```

```{r tabsize, echo=F}

# Table of best tree size
data.frame(Size = tree.size) %>%
  rownames_to_column() %>%
  pivot_longer(-rowname, names_to = "Run") %>%
  pivot_wider(names_from = rowname) %>%
  kable(caption = "\\label{tab:size}Best tree size across 5 runs",
        align = "c") 
```
...
accuracy.df %>%
  round(3) %>%
  kable(caption = "\\label{tab:acc}Accuracy rates for the three classification methods",
...
accuracy.df %>% 
  ...
  summarize_all(mean) %>%
  round(3) %>%
  kable(caption = "\\label{tab:acc2}Average accuracy rates for the three classification methods",
...
```{r figlambda, echo=F, fig.width=8, fig.cap="\\label{fig:lambda}Finding the minimum $\\lambda$ with ten-fold cross-validation (Run 5)"}
...
```{r figk, echo=F, fig.width=8, fig.cap="\\label{fig:k}Finding the optimal k with leave-one-out cross-validation (Run 5)"}
...
```{r figsize, echo=F, fig.width=8, fig.cap="\\label{fig:size}Finding the best size with ten-fold cross-validation (Run 5)"}
