...
cbind(variables,description) %>%
  kable(caption = "\\label{tab:tabdata}Heart Disease Data Set",
...
cbind(whole,by_num) %>%
  print(caption = "\\label{tab:tabdata2}Detailed Summary of the Heart Disease Data Set",
...
```{r figacc, echo=F, fig.width=8, fig.cap="\\label{fig:figacc}Accuracy rates for the three classification methods"}
accuracy.df %>%
...
coef(lr.final, s = "lambda.min") %>%
  ...
  kable(caption = "\\label{tab:tablasso}LASSO-penalized logistic regression model to predict future observations")
```
...
data.frame(Lambda = round(lasso.lambda, 3)) %>%
  ...
  kable(caption = "\\label{tab:tablambda}Minimum values of lambda across 5 runs",
        align = "c") 
```
...
data.frame(k = knn.k) %>%
  ...
  pivot_longer(-rowname, names_to = "Run") %>%
  pivot_wider(names_from = rowname) %>%
  kable(caption = "\\label{tab:tabk}Optimal values of k across 5 runs",
        align = "c") 
```
...
data.frame(Size = tree.size) %>%
  ...
  pivot_longer(-rowname, names_to = "Run") %>%
  pivot_wider(names_from = rowname) %>%
  kable(caption = "\\label{tab:tabsize}Best tree size across 5 runs",
        align = "c") 
```
...
accuracy.df %>%
  round(3) %>%
  kable(caption = "\\label{tab:tabacc}Accuracy rates for the three classification methods",
...
accuracy.df %>% 
  ...
  summarize_all(mean) %>%
  round(3) %>%
  kable(caption = "\\label{tab:tabacc2}Average accuracy rates for the three classification methods",
...
```{r figlambda, echo=F, fig.width=8, fig.cap="\\label{fig:figlambda}Finding the minimum $\\lambda$ with ten-fold cross-validation (Run 5)"}
...
```{r figk, echo=F, fig.width=8, fig.cap="\\label{fig:figk}Finding the optimal k with leave-one-out cross-validation (Run 5)"}
...
```{r figsize, echo=F, fig.width=8, fig.cap="\\label{fig:figsize}Finding the best size with ten-fold cross-validation (Run 5)"}
